...
public class MusicServiceImpl extends AbstractMusicService implements MusicService {
	...
	public void newData() {
		...
		try {
			for (Music music : getCachedMusic(false)) {
				removeMusic(music);
			}
			clearCache();
	...
	public void duplicate(final Music music) {
		...
		try {
			...
			for (Song song : getCachedSongs(music, false)) {
	...
	public int getTotalMediaCount() {
		...
		try {
			int sum = 0;
			for (Music music : getCachedMusic(true)) {
	...
	public Time getTotalLength() {
		...
		try {
			int sum = 0;
			for (Music music : getCachedMusic(true)) {
				for (Song song : getCachedSongs(music, true)) {
					sum += song.getLength();
				}
			}
			return new Time(sum);
	 ...
	 */
	private void switchPosition(final Music music1, final Music music2) {
