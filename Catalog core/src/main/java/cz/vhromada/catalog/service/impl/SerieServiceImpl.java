...
public class SerieServiceImpl extends AbstractSerieService implements SerieService {
	...
	public void newData() {
		...
		try {
			for (Serie serie : getCachedSeries(false)) {
				removeSerie(serie);
			}
			clearCache();
	...
	public void duplicate(final Serie serie) {
		...
		try {
			...
			for (Season season : getCachedSeasons(serie, false)) {
	...
	public Time getTotalLength() {
		...
		try {
			int sum = 0;
			for (Serie serie : getCachedSeries(true)) {
				for (Season season : getCachedSeasons(serie, true)) {
	 ...
	 */
	private Serie createSerie(final Serie serie) {
	 ...
	 */
	private void switchPosition(final Serie serie1, final Serie serie2) {
