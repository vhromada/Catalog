...
class MovieValidator(
    ...
    override fun validateDataDeep(data: Movie, result: Result<Unit>) {
        ...
        when {
            data.year == null -> {
                result.addEvent(Event(Severity.ERROR, "${getPrefix()}_YEAR_NULL", "Year mustn't be null."))
            }
            data.year < Constants.MIN_YEAR || data.year > Constants.CURRENT_YEAR -> {
                result.addEvent(Event(Severity.ERROR, "${getPrefix()}_YEAR_NOT_VALID", "Year must be between ${Constants.MIN_YEAR} and ${Constants.CURRENT_YEAR}."))
            }
        }
    ...
    private fun validateMedia(data: Movie, result: Result<Unit>) {
        if (data.media == null) {
            result.addEvent(Event(Severity.ERROR, "${getPrefix()}_MEDIA_NULL", "Media mustn't be null."))
        } else {
            if (data.media.contains(null)) {
                result.addEvent(Event(Severity.ERROR, "${getPrefix()}_MEDIA_CONTAIN_NULL", "Media mustn't contain null value."))
            }
    ...
    private fun validateUrls(data: Movie, result: Result<Unit>) {
        if (data.csfd == null) {
            result.addEvent(Event(Severity.ERROR, "${getPrefix()}_CSFD_NULL", "URL to ČSFD page about movie mustn't be null."))
        }
        when {
            data.imdbCode == null -> {
        ...
        if (data.wikiEn == null) {
            result.addEvent(Event(Severity.ERROR, "${getPrefix()}_WIKI_EN_NULL", "URL to english Wikipedia page about movie mustn't be null."))
        }
        if (data.wikiCz == null) {
            result.addEvent(Event(Severity.ERROR, "${getPrefix()}_WIKI_CZ_NULL", "URL to czech Wikipedia page about movie mustn't be null."))
        }
    }
