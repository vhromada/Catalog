...
public class BookCategoryServiceImpl extends AbstractBookService implements BookCategoryService {
	...
	@Override
	public void newData() {
		Validators.validateFieldNotNull(bookCategoryDAO, "DAO for book categories");
		Validators.validateFieldNotNull(bookDAO, "DAO for books");
		validateBookCacheNotNull();

		try {
			for (BookCategory bookCategory : getCachedBookCategories(false)) {
	...
	public void duplicate(final BookCategory bookCategory) {
		...
		try {
			...
			for (Book book : getCachedBooks(bookCategory, false)) {
	...
	public int getBooksCount() {
		...
		try {
			int sum = 0;
			for (BookCategory bookCategory : getCachedBookCategories(true)) {
	 ...
	 */
	private void switchPosition(final BookCategory bookCategory1, final BookCategory bookCategory2) {
